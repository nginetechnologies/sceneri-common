...
#pragma clang optimize on
	...
	/* static */ Path Path::GetApplicationDataDirectory()
	{
		static const Path applicationDataDirectory = []()
		{
			...
			return TPath::Combine(ViewType(applicationDataPath, (uint16)wcslen(applicationDataPath)), EngineInfo::AsciiName.GetView());
#elif PLATFORM_APPLE_MACOS
	...
	/* static */ Path Path::GetApplicationCacheDirectory()
	{
		static const Path applicationCacheDirectory = []()
		{
			...
			return TPath::Combine(TPath([cacheDirectory UTF8String], (Path::SizeType)[cacheDirectory length]), EngineInfo::AsciiName.GetView());
#elif PLATFORM_APPLE_IOS || PLATFORM_APPLE_VISIONOS
			NSArray* paths = NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES);
			NSString* cacheDirectory = [paths objectAtIndex:0];
	...
	/* static */ Path Path::GetTemporaryDirectory()
	{
		...
		return TPath::Combine(path.GetView(), EngineInfo::AsciiName.GetView());
#elif PLATFORM_APPLE
		...
		return TPath::Combine(temporaryDirectory, EngineInfo::AsciiName.GetView());
#elif PLATFORM_ANDROID
	...
	/* static */ Path Path::GetUserDataDirectory()
	{
		static const Path userDataDirectory = []()
		{
			...
			return TPath::Combine(TPath::ViewType(documentsPath, (uint16)wcslen(documentsPath)), EngineInfo::AsciiName.GetView());
#elif PLATFORM_APPLE_MACOS
