...
#include <Common/Reflection/PropertyFlags.h>
...
namespace ngine::Reflection
{
	...
	namespace Internal
	{
		...
		{
			GetTypeSize,
			GetHasDynamicCompressedData,
			CalculateFixedCompressedDataSize,
			GetTypeAlignment,
			GetTypeName,
			...
			CalculateObjectDynamicCompressedDataSize,
			CompressStoredObject,
			DecompressStoredObject
		};
	...
	public:
		...
		[[nodiscard]] bool HasDynamicCompressedData(EnumFlags<Reflection::PropertyFlags> requiredFlags) const;
		[[nodiscard]] uint32 CalculateFixedCompressedDataSize(EnumFlags<Reflection::PropertyFlags> requiredFlags) const;
		[[nodiscard]] uint32
		CalculateObjectDynamicCompressedDataSize(const ConstStoredType source, EnumFlags<Reflection::PropertyFlags> requiredFlags) const;
		[[nodiscard]] bool CompressStoredObject(
			const ConstStoredType source, BitView& target, const EnumFlags<Reflection::PropertyFlags> requiredFlags = {}
		) const;
		[[nodiscard]] bool DecompressStoredObject(
			const StoredType target, ConstBitView& source, const EnumFlags<Reflection::PropertyFlags> requiredFlags = {}
		) const;
