	...
	void GenericTypeDefinition<Type>::Manage(const Operation operation, OperationArgument& argument)
	{
		switch (operation)
		{
			...
			case Operation::GetHasDynamicCompressedData:
			{
				...
				*reinterpret_cast<bool*>(argument.m_typeArgs[1]) = Compression::IsDynamicallyCompressed<FilteredType>(requiredFlags);
			}
			break;
			case Operation::CalculateFixedCompressedDataSize:
			{
				...
				const EnumFlags<Reflection::PropertyFlags> requiredFlags =
					*reinterpret_cast<EnumFlags<Reflection::PropertyFlags>*>(argument.m_typeArgs[0]);
				*reinterpret_cast<uint32*>(argument.m_typeArgs[1]) = Compression::CalculateFixedDataSize<FilteredType>(requiredFlags);
			}
			break;
			...
			case Operation::CalculateObjectDynamicCompressedDataSize:
			{
				...
				*reinterpret_cast<uint32*>(argument.m_storedObjectArgs.m_args[1]
				) = Compression::CalculateDynamicDataSize<FilteredType>(value, requiredFlags);
			}
			break;
			case Operation::CompressStoredObject:
			{
				...
				const bool success = Compression::Compress<FilteredType>(value, targetView, requiredFlags);
				argument.m_storedObjectArgs.pObject = reinterpret_cast<void*>(success);
			}
			break;
			case Operation::DecompressStoredObject:
			{
				...
				const EnumFlags<Reflection::PropertyFlags> requiredFlags =
					*reinterpret_cast<EnumFlags<Reflection::PropertyFlags>*>(argument.m_storedObjectArgs.m_args[1]);
				const bool success = Compression::Decompress<FilteredType>(value, sourceView, requiredFlags);
				argument.m_storedObjectArgs.pObject = reinterpret_cast<void*>(success);
			}
			break;
		}
